...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font='Arial.ttf', pil=False, example='abc'):
        ...
        if self.pil:  # use PIL
            ...
            check_font(font='Arial.Unicode.ttf' if is_chinese(example) else font,
...
def distancing(people_coords, img, dist_thres_lim=(200, 250)):
    ...
    for x in x_combs:
        ...
        if dist > dist_thres_lim[0] and dist < dist_thres_lim[1]:
            if not xyxy1 in low_li:
                low_li.append(xyxy1)
            if not xyxy2 in low_li:
                low_li.append(xyxy2)
            color = (0, 255, 255)
            label = "Low Risk "
            ...
            if not xyxy1 in high_li:
                high_li.append(xyxy1)
            if not xyxy2 in high_li:
                high_li.append(xyxy2)
            color = (0, 0, 255)
            label = "High Risk"
            already_red[cntr1] = 1
            already_red[cntr2] = 1
            ...
            for xy in x:
                ...
                cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf,
                            lineType=cv2.LINE_AA)
    cv2.putText(img, "High Risk:%d" % len(high_li), (10, 30), 0, 1, [225, 0, 0], 2, lineType=cv2.LINE_AA)
    cv2.putText(img, "Low Risk:%d" % len(low_li), (10, 60), 0, 1, [225, 0, 0], 2, lineType=cv2.LINE_AA)